...
import { requireSessionJudge } from '@/lib/auth/session';
...
export async function POST(request: Request) {
  try {
    ...
    // Load group first to get sessionId for session-binding check
    ...
    const authToken = requireSessionJudge(request, group.session_id);

    // Validate judge identity
    if (judgeId !== authToken.judgeId) {
    ...
    if (insertError) {
      console.error('scores.submit', insertError);
      return NextResponse.json({ error: 'Failed to submit scores' }, { status: 500 });
    }

    const { error: subError } = await supabaseAdmin
      .from('score_submissions')
      .insert({ group_id: groupId, judge_id: judgeId, score_count: scores.length });

    if (subError) {
      console.error('scores.submit.record', subError);
